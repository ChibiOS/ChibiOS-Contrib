...
static void cmd_date(BaseSequentialStream *chp, int argc, char *argv[]) {
  ...
  rtcGetTime(&PORTAB_RTC1, &timespec);
...
static void cmd_storage(BaseSequentialStream *chp, int argc, char *argv[]) {
  size_t storage_size = psGetStorageSize(&PORTAB_RTC1);
  ps_offset_t i;
 ...
 * ERTC callback.
 */
static void alarmcb(RTCDriver *rtcp, ertcevent_t event) {

  (void)rtcp;

  if (event == ERTC_EVENT_ALARM_A) {
    interval = TIME_MS2I(500);
  }
  else if (event == ERTC_EVENT_ALARM_B) {
...
int main(void) {
  static const RTCAlarm alarm1 = {
    ERTC_ALRM_MASK4 |   /* No month/week day match. */
    ERTC_ALRM_MASK3 |   /* No hour match.           */
    ERTC_ALRM_MASK2 |   /* No minutes match.        */
    ERTC_ALRM_ST(0) |
    ERTC_ALRM_SU(0)     /* Match minute start.      */
  };
  static const RTCAlarm alarm2 = {
    ...
    ERTC_ALRM_MASK2 |   /* No minutes match.        */
    ERTC_ALRM_ST(3) |
    ERTC_ALRM_SU(0)     /* Match minute half.       */
  };
  ...
  rtcSetAlarm(&PORTAB_RTC1, 0, &alarm1);
  rtcSetAlarm(&PORTAB_RTC1, 1, &alarm2);
  rtcSetCallback(&PORTAB_RTC1, alarmcb);
#if RTC_HAS_STORAGE
  psWrite(&PORTAB_RTC1, 0U, 12U, (const uint8_t *)"Hello World!");
#endif
