...
static uint16_t freq1, freq2; /* Final values in Hz */
...
void captureOverflowCb(TIMCAPDriver *timcapp)
{
  ...
  {
    timcapp->tim->DIER &= ~TIM_DIER_CC1IE; /* Disable CC1 IRQ */
  }

  if (TIM3CC2UD && (timcapp->tim->DIER & TIM_DIER_CC2IE))
  {
    timcapp->tim->DIER &= ~TIM_DIER_CC2IE; /* Disable CC2 IRQ */
  }
  /* Indicate we have reached the auto-reload value twice */
  TIM3CC1UD = true;
  TIM3CC2UD = true;
}
...
void capture1Cb(TIMCAPDriver *timcapp)
{
  ...
  else if(TIM3CC1CaptureNumber == 1)
  {
      ...
      freq1 = (tc_conf.frequency / Capture);
...
void capture2Cb(TIMCAPDriver *timcapp)
{
  ...
  else if(TIM3CC2CaptureNumber == 1)
  {
      ...
      /* Frequency computation */
      freq2 = (tc_conf.frequency / Capture);
...
int main(void) {
   ...
   */
  while (true) {
	reEnableInputCapture(&TIMCAPD3); /* Re-enable every 100ms */
