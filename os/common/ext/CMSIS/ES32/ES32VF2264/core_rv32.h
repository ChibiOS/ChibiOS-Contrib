...
*/
#define _VAL2FLD_(field, value)    ((value << field ## _Pos) & field ## _Msk)
...
*/
#define _FLD2VAL_(field, value)    ((value & field ## _Msk) >> field ## _Pos)
...
#define CLIC                ((ES_CLIC_Type    *)     CLIC_BASE   )       /*!< CLIC configuration struct */
...
__STATIC_INLINE void csi_vic_enable_sirq(int32_t IRQn)
{
	...
	CLIC->CLICINT[IRQn].ATTR |= CLIC_INTATTR_SHV_Msk; 
}
                                           ...
                                           mpu_region_attr_t attr, uint32_t enable)
{
    ...
    if (attr.a == ADDRESS_MATCHING_TOR) {
        ...
        if (size == REGION_SIZE_4B) {
            addr = base_addr >> 2;
            attr.a = ADDRESS_MATCHING_NA4;
        } else {
            addr = ((base_addr >> 2) & (0xFFFFFFFFU - ((1 << (size + 1)) - 1))) | ((1 << size) - 1);
        }
    }

    __es_set_PMPADDRx(idx, addr);
...
{
    __es_set_PMPxCFG(idx, __es_get_PMPxCFG(idx) & (~PMP_PMPCFG_A_Msk));
}
...
__STATIC_INLINE uint32_t csi_coret_clr(uint32_t ticks, int32_t IRQn)
{
    CORET->MTIMECMP = CORET->MTIME + ticks;
...
__STATIC_INLINE int32_t csi_had_receive_char(void)
{
    ...
    if (_FLD2VAL_(DCC_EHSR_JW, DCC->EHSR)) {
        ch = DCC->DERJW;
    }
...
{
    return _FLD2VAL_(DCC_EHSR_JW, DCC->EHSR);                              /* no character available */
}
...
#define MEXSTATUS_WFE_Pos                             4U
#define MEXSTATUS_WFE_Msk                        (0x1UL << MEXSTATUS_WFE_Pos) 

typedef enum {
    MEXSTATUS_RESET_CORE   = 1,    /*  Reset CPU Core, But Not Reset Debug System */
    MEXSTATUS_RESET_CPUSYS = 2     /*  Reset CPU Core and  Reset Debug System */ 
} mexstatus_reset_e;

typedef enum {
    MEXSTATUS_SLEEP_LIGHT   = 1,
    MEXSTATUS_SLEEP_DEEP    = 0
} mexstatus_sleep_e;
...
/**
  \brief   CPU Reset
  \details Triggle CPU Reset
  */
__STATIC_INLINE uint32_t csi_cpu_get_reset_addr (void)
{
    uint32_t result;
    result = __get_MRADDR();
...
/**
  ...
  \details Triggle CPU Reset
  */
__STATIC_INLINE void csi_cpu_reset (mexstatus_reset_e reset_mode)
{
    uint32_t mexstatus;
    mexstatus = __get_MEXSTATUS();
    mexstatus &= (~(MEXSTATUS_RESET_Msk));
    mexstatus |= (uint32_t)(reset_mode << MEXSTATUS_RESET_Pos);
    __set_MEXSTATUS(mexstatus);
}

/**
  \brief   CPU Sleep WFI mode
  \details Triggle CPU Sleep
  */
__STATIC_INLINE void csi_cpu_sleep_wfi (mexstatus_sleep_e sleep_mode)
{
    uint32_t mexstatus;
    mexstatus = __get_MEXSTATUS();
    mexstatus &= (~(MEXSTATUS_SLEEP_Msk));
    mexstatus &= (~(MEXSTATUS_WFE_Msk));   /* WFI mode */
    mexstatus |= (uint32_t)(sleep_mode << MEXSTATUS_SLEEP_Pos);
    __set_MEXSTATUS(mexstatus);
}

/**
  \brief   CPU Sleep WFE mode
  \details Triggle CPU Sleep
  */
__STATIC_INLINE void csi_cpu_sleep_wfe (mexstatus_sleep_e sleep_mode)
{
    uint32_t mexstatus;
    mexstatus = __get_MEXSTATUS();
    mexstatus &= (~(MEXSTATUS_SLEEP_Msk));
    mexstatus |= MEXSTATUS_WFE_Msk;   /* WFE mode */
    mexstatus |= (uint32_t)(sleep_mode << MEXSTATUS_SLEEP_Pos);
    __set_MEXSTATUS(mexstatus);
}

/**
  \brief  Get CPU Sleep Status
  \details Get CPU Sleep Status
  */
__STATIC_INLINE uint32_t csi_cpu_get_sleep_status (void)
{
    uint32_t mexstatus;
    mexstatus = __get_MEXSTATUS();
    mexstatus = (mexstatus_sleep_e)((mexstatus & MEXSTATUS_SLEEP_Msk) >> MEXSTATUS_SLEEP_Pos);
    return mexstatus;
}
