...
void ald_dma_config_base(ald_dma_config_t *config)
{
	...
	{
        SET_BIT(DMA->CHANNEL[config->channel].CON, DMA_CON_DIR_MSK);
	}
    else
    {
        CLEAR_BIT(DMA->CHANNEL[config->channel].CON, DMA_CON_DIR_MSK);
    }
...
/**
  * @}DMA_Public_Functions_Group2
  */

/**
  * @}DMA_Public_Functions
  */
