...
static void uart_dma_recv_cplt(void *arg)
{
    ...
    ald_uart_handle_t *hperh = (ald_uart_handle_t *)arg;
    hperh->rx_count = hperh->rx_size;

    ald_uart_dma_req_config(hperh, ALD_UART_DMA_REQ_RX, DISABLE);
    hperh->rx_state = ALD_UART_STATE_READY;
...
{
    uint32_t cnt = 0xFFFFFFU;

    if (hperh->state != ALD_UART_STATE_BUSY_TX)
        return ALD_BUSY;
    ...
    if (hperh->tx_count >= hperh->tx_size)
    {
        ...
        /* Wait TX empty */
        cnt = 0xFFFFFFU;
        while (!(hperh->perh->STAT & (0x1 << 16)) && (cnt--));
...
static ald_status_t __uart_recv_by_it(ald_uart_handle_t *hperh)
{
    ...
    if (hperh->rx_state != ALD_UART_STATE_BUSY_RX)
	{
        return ALD_BUSY;
	}
	...
	{
		tmp = (uint8_t)(hperh->perh->RXBUF & 0xFF);
		UNUSED(tmp);
	}
...
ald_status_t ald_uart_send(ald_uart_handle_t *hperh, uint8_t *buf, uint16_t size, uint32_t timeout)
{
    ...
    if (hperh->state != ALD_UART_STATE_READY)
        return ALD_BUSY;
    ...
    __LOCK(hperh);
    hperh->err_code = ALD_UART_ERROR_NONE;
    hperh->state = ALD_UART_STATE_BUSY_TX;
...
ald_status_t ald_uart_recv(ald_uart_handle_t *hperh, uint8_t *buf, uint16_t size, uint32_t timeout)
{
    ...
    if (hperh->rx_state != ALD_UART_STATE_READY)
        return ALD_BUSY;
    ...
    __LOCK(hperh);
    hperh->err_code = ALD_UART_ERROR_NONE;
    hperh->rx_state = ALD_UART_STATE_BUSY_RX;
...
void ald_uart_irq_handler(ald_uart_handle_t *hperh)
{
	...
	{
        hperh->err_code |= ALD_UART_ERROR_ORE;
	    hperh->perh->ICR = ALD_UART_IF_RFOERR;
	}
	...
	{
        hperh->err_code |= ALD_UART_ERROR_ORE;
	    hperh->perh->ICR = ALD_UART_IF_TFOVER;
	}
...
void ald_uart_irq_handler_fast(ald_uart_handle_t *hperh)
{
    ...
    if (tmp & 0x10000)
    {
        ...
        if (hperh->tx_count >= hperh->tx_size)
        {
            ...
            /* Wait for fifo empty */
            while (!(hperh->perh->STAT & 0x10000U) && (cnt--));

            /* Enable TBC interrupt */
            hperh->perh->IER = 0x4000;
            hperh->state = ALD_UART_STATE_READY;
    ...
    if (tmp & 0x400)
    {
		...
		{
			tmp_reg = (uint8_t)(hperh->perh->RXBUF & 0xFF);
			UNUSED(tmp_reg);
		}
    ...
    if (tmp & 0x10)
    {
        hperh->perh->ICR = 0x10U;
        hperh->perh->ICR = 0x400U;

        hperh->perh->IDR = 0x10U;
        hperh->perh->IDR = 0x400U;
        hperh->rx_state = ALD_UART_STATE_READY;
