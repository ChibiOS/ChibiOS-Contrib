 ...
 * @retval None
 */
void ald_wwdt_init_no_reset(uint32_t load, ald_wwdt_win_t win, type_func_t interrupt)
{
    assert_param(IS_WWDT_WIN_TYPE(win));
    assert_param(IS_FUNC_STATE(interrupt));
...
/**
 * @brief Initializes the IWDG without reset according to the specified parameters.
 * @param load: Specifies the free-running downcounter value.
 * @param interrupt: Enable or disable interrupt.
 * @retval None
 */
void ald_iwdt_init_no_reset(uint32_t load, type_func_t interrupt)
{
    assert_param(IS_FUNC_STATE(interrupt));
