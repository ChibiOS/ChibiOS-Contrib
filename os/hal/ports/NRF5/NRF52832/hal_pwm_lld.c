...
#if 0
  if ((pwmp->pwm->INTEN & PWM_INTEN_LOOPSDONE_Msk) && (pwmp->pwm->EVENTS_LOOPSDONE)) {
	  pwmp->pwm->EVENTS_LOOPSDONE = 0;
	  (void)pwmp->pwm->EVENTS_LOOPSDONE;

      pwmp->config->callback(pwmp);
  }
  if ((pwmp->pwm->INTEN & PWM_INTEN_SEQEND0_Msk) && (pwmp->pwm->EVENTS_SEQEND[0])) {
	  pwmp->pwm->EVENTS_SEQEND[0] = 0;
	  (void)pwmp->pwm->EVENTS_SEQEND[0];

      pwmp->config->callback(pwmp);
  }
  if ((pwmp->pwm->INTEN & PWM_INTEN_SEQEND1_Msk) && (pwmp->pwm->EVENTS_SEQEND[1])) {
