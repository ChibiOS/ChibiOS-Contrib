...
static uint32_t wait_for_clock_ready_2(uint32_t clkMask)
{
  // int32_t timeout = 2180000;
  int32_t timeout = 221200;
...
static void set_HCLK_2(uint32_t clkSource, uint32_t clkDivider)
{
  /* Set new clock divider */
  CLK->CLKDIV = (CLK->CLKDIV & (~CLK_CLKDIV_HCLK_N_Msk)) | clkDivider;
...
static uint32_t set_core_clock_2(uint32_t clkCore)
{
  /* Use __HIRC as HCLK temporarily */
  CLK->CLKSEL0 |= CLK_CLKSEL0_HCLK_S_Msk;
  CLK->CLKDIV &= (~CLK_CLKDIV_HCLK_N_Msk);
  ...
  CLK->PLLCON = CLK_PLLCON_PD_Msk;
    ...
    /* Use NUC123_HSECLK as PLL source */
    // clkCore = enable_pll(NUC123_PLLSRC_HSE, (4 * clkCore));
    /*
         * The default value: 0xC22E
         *   FIN = 12 MHz
         *   NR = (1+2) = 3
...
#else
    /* Use __HIRC as PLL source */
    // clkCore = enable_pll(NUC123_PLLSRC_HSI, (4 * clkCore));
    /*
         * The default value: 0xD66F
         *   FIN = 22.1184 MHz
         *   NR = (11+2) = 13
         *   NF = (111+2) = 113
         *   NO = 4
         *   FOUT = 192.26... MHz x 1/4 = 48.0... MHz
         */
      CLK->PLLCON = NUC123_PLLSRC_HSI | 0xD66F;
      clkCore = 48064985;
#endif

  wait_for_clock_ready_2(CLK_CLKSTATUS_PLL_STB_Msk);

  /* Set HCLK clock source to PLL */
  set_HCLK_2(NUC123_HCLKSRC_PLL, CLK_CLKDIV_HCLK(1));
...
{
  // PA->PMD = (PA->PMD & 0xFFCFFFFF) | 0x00100000;
  // PA->DOUT = (PA->DOUT & 0xFFFFFBFF);
...
void NUC123_clock_init(void)
{
  ...
  // if (((*(volatile uint32_t *)0x5000020C) & 16) == 0){
  // if (((*(volatile uint32_t *)0x50000210) & 7) == 7){
  ...
  // uint32_t t = wait_for_clock_ready_2(CLK_CLKSTATUS_OSC22M_STB_Msk);
  ...
  set_HCLK_2(NUC123_HCLKSRC_HSE, CLK_CLKDIV_HCLK(1));
