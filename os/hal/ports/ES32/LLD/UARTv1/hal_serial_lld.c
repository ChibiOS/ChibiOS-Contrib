...
#    if ES32_SERIAL_USE_USART0 == TRUE
...
#    if ES32_SERIAL_USE_USART1 == TRUE
...
static void usartInit(SerialDriver *sdp, const SerialConfig *config)
{
    ...
    u->DLR = (uint32_t)(ES32_CK_USART_FREQUENCY / config->speed);                      /* Data Rate */
...
#    if ES32_SERIAL_USE_USART0 == TRUE
CH_IRQ_HANDLER(ES32_USART0_IRQ_VECTOR)
{
    CH_IRQ_PROLOGUE();
    serialInterrupt(&SD0);
    CH_IRQ_EPILOGUE();
...
#    if ES32_SERIAL_USE_USART1 == TRUE
CH_IRQ_HANDLER(ES32_USART1_IRQ_VECTOR)
{
    CH_IRQ_PROLOGUE();
    serialInterrupt(&SD1);
    CH_IRQ_EPILOGUE();
...
#    endif  // ES32_SERIAL_USE_USART0 == TRUE
#    if ES32_SERIAL_USE_USART1 == TRUE
...
#    endif  // ES32_SERIAL_USE_USART1 == TRUE
    }
}
