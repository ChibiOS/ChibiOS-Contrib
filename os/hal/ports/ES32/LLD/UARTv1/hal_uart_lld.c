...
#if (ES32_UART_USE_UART0 == TRUE) || defined(__DOXYGEN__)
OSAL_IRQ_HANDLER(ES32_UART0_IRQ_VECTOR)
{
    OSAL_IRQ_PROLOGUE();
...
#endif
#if (ES32_UART_USE_UART1 == TRUE) || defined(__DOXYGEN__)
OSAL_IRQ_HANDLER(ES32_UART1_IRQ_VECTOR)
{
    OSAL_IRQ_PROLOGUE();
...
#if (ES32_UART_USE_USART0 == TRUE) || defined(__DOXYGEN__)
OSAL_IRQ_HANDLER(ES32_USART0_IRQ_VECTOR)
{
    OSAL_IRQ_PROLOGUE();
...
#endif
#if (ES32_UART_USE_USART1 == TRUE) || defined(__DOXYGEN__)
OSAL_IRQ_HANDLER(ES32_USART1_IRQ_VECTOR)
{
    OSAL_IRQ_PROLOGUE();
...
#if ES32_UART_USE_UART0 == TRUE
    uartObjectInit(&UARTD0);
    UARTD0.UART = UART0;
#endif
#if ES32_UART_USE_UART1 == TRUE
    uartObjectInit(&UARTD1);
    UARTD1.UART = UART1;
#endif
#if ES32_UART_USE_USART0 == TRUE
    uartObjectInit(&USARTD0);
    USARTD0.UART = USART0;
#endif
#if ES32_UART_USE_USART1 == TRUE
    uartObjectInit(&USARTD1);
    USARTD1.UART = USART1;
#endif

    /* Peripheral initialization. */
    CKCU->GCFGR = (CKCU->GCFGR & ~CKCU_GCFGR_URPRE_MASK) | ((ES32_USART_PRESCALER - 1) << 22);
}
...
void uart_lld_start(UARTDriver *uartp)
{
    if (uartp->state == UART_STOP)
    {
        ...
        if (&UARTD1 == uartp)
        {
            ...
            nvicEnableVector(UART0_IRQn, ES32_UART0_IRQ_PRIORITY);
        }
        ...
        if (&UARTD1 == uartp)
        {
            ...
            nvicEnableVector(UART1_IRQn, ES32_UART1_IRQ_PRIORITY);
        }
        ...
        if (&USARTD0 == uartp)
        {
            ...
            nvicEnableVector(USART0_IRQn, ES32_USART0_IRQ_PRIORITY);
        }
        ...
        if (&USARTD1 == uartp)
        {
            ...
            nvicEnableVector(USART1_IRQn, ES32_USART1_IRQ_PRIORITY);
        }
    ...
    uartp->UART->DLR = ES32_CK_AHB_FREQUENCY / uartp->config->baud;
}
