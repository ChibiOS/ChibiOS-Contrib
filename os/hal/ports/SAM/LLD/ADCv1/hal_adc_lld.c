  ...
  if (adcp->grpp != NULL) {
    ...
    if ((isr & ADC_INTFLAG_RESRDY_Msk) && (adcp->state == ADC_ACTIVE)) {
      ...
      if (adcp->curr_size == adcp->depth * adcp->grpp->num_channels) {
        ...
        adcp->curr_size += 1;
        ADCD1.adc->ADC_INTENSET =
            ADC_INTENSET_OVERRUN_Msk | ADC_INTENSET_RESRDY_Msk;
        uint8_t curr_pin = adcp->grpp->seq[adcp->curr_pin];
        adcp->adc->ADC_INPUTCTRL =
            (adcp->grpp->inputctrl &
             ~(ADC_INPUTCTRL_MUXPOS_Msk | ADC_INPUTCTRL_INPUTSCAN_Msk)) |
            ADC_INPUTCTRL_MUXPOS(curr_pin);
...
void adc_lld_start(ADCDriver* adcp) {
  ...
  if (adcp->state == ADC_STOP) {
    ...
    if (&ADCD1 == adcp) {
      ...
      while ((adcp->adc->ADC_STATUS & ADC_STATUS_SYNCBUSY_Msk) != 0U)
        ;
      uint32_t adc_linearity0 =
          (((*(uint32_t*)OTP4_ADDR) & ADC_LINEARITY0_Msk) >>
           ADC_LINEARITY0_POS);
      uint32_t adc_linearity1 =
          (((*(uint32_t*)(OTP4_ADDR + 4U)) & ADC_LINEARITY1_Msk) >>
           ADC_LINEARITY1_POS);
      adcp->adc->ADC_CALIB =
          (uint16_t)((ADC_CALIB_LINEARITY_CAL(adc_linearity0 |
                                              (adc_linearity1 << 5U))) |
                     ADC_CALIB_BIAS_CAL(
                         (((*(uint32_t*)(OTP4_ADDR + 4U)) & ADC_BIASCAL_Msk) >>
                          ADC_BIASCAL_POS)));
      while ((adcp->adc->ADC_STATUS & ADC_STATUS_SYNCBUSY_Msk) != 0U)
        ;
...
void adc_lld_start_conversion(ADCDriver* adcp) {
  ...
  adcp->adc->ADC_INPUTCTRL =
      (adcp->grpp->inputctrl &
       ~(ADC_INPUTCTRL_MUXPOS_Msk | ADC_INPUTCTRL_INPUTSCAN_Msk)) |
      ADC_INPUTCTRL_MUXPOS(adcp->grpp->seq[adcp->curr_pin]);
  while ((adcp->adc->ADC_STATUS & ADC_STATUS_SYNCBUSY_Msk) != 0U)
    ;
  adcp->curr_pin += 1;
  ...
  while ((adcp->adc->ADC_STATUS & ADC_STATUS_SYNCBUSY_Msk) != 0U)
    ;
  adcp->adc->ADC_INTENSET =
      (uint8_t)ADC_INTENSET_OVERRUN_Msk | ADC_INTENSET_RESRDY_Msk;
