...
static void sam_xosc32_init(void) {
  ...
  SYSCTRL_REGS->SYSCTRL_XOSC32K = SYSCTRL_XOSC32K_STARTUP(5) |
  ...
  while ((SYSCTRL_REGS->SYSCTRL_PCLKSR & SYSCTRL_PCLKSR_XOSC32KRDY_Msk) !=
    ...
    ;
  GCLK_REGS->GCLK_GENCTRL = GCLK_GENCTRL_ID(1) | GCLK_GENCTRL_SRC_XOSC32K |
                            GCLK_GENCTRL_IDC(1) | GCLK_GENCTRL_GENEN_Msk;
...
static void sam_xosc_init(void) {
  ...
  SYSCTRL_REGS->SYSCTRL_XOSC = SYSCTRL_XOSC_STARTUP(5) |
                               SYSCTRL_XOSC_XTALEN_Msk | SYSCTRL_XOSC_AMPGC_Msk;
  ...
  while ((SYSCTRL_REGS->SYSCTRL_PCLKSR & SYSCTRL_PCLKSR_XOSCRDY_Msk) !=
    ...
    ;
  GCLK_REGS->GCLK_GENCTRL = GCLK_GENCTRL_ID(1) | GCLK_GENCTRL_SRC_XOSC |
                            GCLK_GENCTRL_IDC(1) | GCLK_GENCTRL_GENEN_Msk;
  while ((GCLK_REGS->GCLK_STATUS & GCLK_STATUS_SYNCBUSY_Msk) ==
         GCLK_STATUS_SYNCBUSY_Msk)
    ;
...
static void sam_dfll48m_init() {
  ...
  SYSCTRL_REGS->SYSCTRL_DFLLVAL =
      SYSCTRL_DFLLVAL_COARSE(calibCoarse) | SYSCTRL_DFLLVAL_FINE(calibFine);
  GCLK_REGS->GCLK_CLKCTRL =
      GCLK_CLKCTRL_GEN_GCLK1 | GCLK_CLKCTRL_CLKEN_Msk | GCLK_CLKCTRL_ID_DFLL48;
  while ((SYSCTRL_REGS->SYSCTRL_PCLKSR & SYSCTRL_PCLKSR_DFLLRDY_Msk) !=
         SYSCTRL_PCLKSR_DFLLRDY_Msk)
    ;

  SYSCTRL_REGS->SYSCTRL_DFLLMUL = SYSCTRL_DFLLMUL_MUL(1464) |
