 ...
 * @brief   Renesas SST26VF064 serial flash driver header.
 *
 * @addtogroup SST26VF064
 * @{
 */
  ...
  */
 #if !defined(SST26VF_SWITCH_WIDTH) || defined(__DOXYGEN__)
 #define SST26VF_SWITCH_WIDTH                   TRUE
 #endif
 
 /**
  * @brief   Device bus mode to be used.
  * @note    if @p SST26VF_SWITCH_WIDTH is @p FALSE then this is the bus mode
  *          that the device is expected to be using.
  * @note    if @p SST26VF_SWITCH_WIDTH is @p TRUE then this is the bus mode
  *          that the device will be switched in.
  * @note    This option is only valid in WSPI bus mode.
  */
 #if !defined(SST26VF_BUS_MODE) || defined(__DOXYGEN__)
 #define SST26VF_BUS_MODE                       SST26VF_BUS_MODE_WSPI4L
 #endif
  ...
  */
 #if !defined(SST26VF_NICE_WAITING) || defined(__DOXYGEN__)
 #define SST26VF_NICE_WAITING                   TRUE
 #endif
 
 /**
  * @brief   Uses 4kB sub-sectors rather than 64kB sectors.
  */
 #if !defined(SST26VF_USE_SUB_SECTORS) || defined(__DOXYGEN__)
 #define SST26VF_USE_SUB_SECTORS                TRUE
 #endif
  ...
  */
 #if !defined(SST26VF_COMPARE_BUFFER_SIZE) || defined(__DOXYGEN__)
 #define SST26VF_COMPARE_BUFFER_SIZE            32
 #endif
  ...
  *          operations.
  */
 #if !defined(SST26VF_READ_DUMMY_CYCLES) || defined(__DOXYGEN__)
 #define SST26VF_READ_DUMMY_CYCLES              8
 #endif
...
// As far as I know they are not applicable to the at25ff but I've only looked at them briefly. 
 #if (SST26VF_COMPARE_BUFFER_SIZE & (SST26VF_COMPARE_BUFFER_SIZE - 1)) != 0
 #error "invalid SST26VF_COMPARE_BUFFER_SIZE value"
 #endif
 
 #if (SST26VF_READ_DUMMY_CYCLES < 1) || (SST26VF_READ_DUMMY_CYCLES > 15)
 #error "invalid SST26VF_READ_DUMMY_CYCLES value (1..15)"
 #endif
 
 #if (SST26VF_BUS_MODE == SST26VF_BUS_MODE_WSPI4L) || defined(__DOXYGEN__)
 /**
 ...
 #elif SST26VF_BUS_MODE == SST26VF_BUS_MODE_WSPI1L
 ...
 #else
 #error "invalid SST26VF_BUS_MODE setting"
 #endif
 ...
 extern "C" {
   ...
   flash_error_t snor_device_read_sfdp(SNORDriver *devp, flash_offset_t offset,
                                       size_t n, uint8_t *rp);
   void snor_device_set_status_register(SNORDriver *devp, uint8_t *sts);
   void snor_device_get_status_register(SNORDriver *devp, uint8_t *sts);
